...
def get_word_score(word, n):
    ...
    for letter in word_list:
        val = SCRABBLE_LETTER_VALUES[letter]
        letter_sum += val    
    score = letter_sum * (7*word_length - 3*(n-word_length))
    if score < 0:
        score = letter_sum * 1
    return score
...
def update_hand(hand, word):
    ...
    hand_copy = hand.copy()
    new_hand = dict()
    word_list = list(str.lower(word))
    for letter in word_list:
        if letter in hand_copy:
            hand_copy[letter] -= 1
    for key in hand_copy:
        if hand_copy[key] > 0:
            new_hand.update({key: hand_copy[key]})        
    return new_hand
#
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    """
    hand_len, total_score = calculate_handlen(hand), 0
    while hand_len > 0:
        ...
        if user_input == "!!":
            ...
            if is_valid_word(user_input,hand, word_list):
                points = get_word_score(user_input, hand_len)
                total_score += points
                print('"%s" earned %d points. Total: %s\n'%(user_input,points,total_score))
            else:
                print('That is not a vaild word. Please choose another word.\n')
